...
export default function CryptoChatbot() {
  ...
  useEffect(() => {
    const storedAuth = localStorage.getItem("tradingAuth");
    ...
    if (storedAuth) {
      try {
        ...
        if (authData?.session_id) {
          return; // Session is valid, do nothing
        }
      } catch (error) {
        console.error("Error parsing tradingAuth:", error);
      }
    }

    // Redirect to dashboard if no valid session
  ...
  //   try {
  //     // Send the user's message to the chat API and get a response
  ...
  //     // Extract the bot's response, or use a fallback message if it's not available
  //     const botReply =
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "/api/proxi", // This will route through your proxy endpoint
        {
          user_input: message // Send the user's message
        }
      );
